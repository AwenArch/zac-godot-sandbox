...
grep -E "SCRIPT ERROR|ERROR:" "$LOG" | grep -vE "leaked at exit|resources still in use|ObjectDB instances|RIDs of type" && FAIL=1
...
SMOKE=$?
grep -E "SCRIPT ERROR|ERROR:|LOAD FAIL" "$LOG" | grep -vE "leaked at exit|resources still in use|ObjectDB instances|RIDs of type" && FAIL=1
[ "$SMOKE" -ne 0 ] && FAIL=1
 
echo "== 4/4 unit tests =="
timeout 300 godot --headless -s addons/gdUnit4/bin/GdUnitCmdTool.gd --ignoreHeadlessMode --add res://tests/unit > "$LOG" 2>&1 || FAIL=1
grep -E "FAILED|Parse Error|SCRIPT ERROR|ERROR:" "$LOG" | grep -vE "leaked at exit|resources still in use|ObjectDB instances|RIDs of type" && FAIL=1
